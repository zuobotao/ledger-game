...
async function assertDreamSelector(page: import('@playwright/test').Page) {
  ...
  await expect(page.getByTestId('random-dream-all')).toHaveAttribute('data-action', 'random-all')
  await expect(page.getByTestId('random-dream-all')).toHaveAccessibleName('所有信息全部随机')
  await page.getByTestId('random-dream-all').click()
  await expect(page.getByTestId('dream-info-0')).toBeEnabled()
  await expect(page.getByTestId('dream-info-1')).toBeEnabled()
  await expect(page.getByTestId('open-career-selector-0')).not.toHaveText('随机职业')
  await expect(page.locator('#player-color-0')).not.toHaveValue('random')
  await expect(page.locator('#player-type-0')).toHaveValue(/^(human|easy|medium|hard)$/)
