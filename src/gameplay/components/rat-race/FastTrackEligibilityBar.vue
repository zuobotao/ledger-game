...
watch(
  ...
  (eligible) => {
    if (eligible && !isMobile.value) expanded.value = true
  },
  { immediate: true },
)
</script>
...
<template>
  ...
  >
    ...
    <div class="flex items-center gap-2">
      <button
        type="button"
        class="flex min-w-0 flex-1 items-center justify-between gap-2 px-3 py-2 text-left transition hover:bg-muted/30"
      ...
      <button
        v-if="eligibility?.eligible && isMobile"
        type="button"
        data-testid="enter-fast-track-cta"
        class="mr-2 inline-flex shrink-0 items-center gap-1 rounded-lg bg-emerald-500 px-2.5 py-1.5 text-[11px] font-semibold text-white transition hover:bg-emerald-600"
        @click="emit('enter')"
      >
        进入
    ...
    <!-- 展开详情 -->
    <div v-if="showDetails" class="border-t border-border px-3 py-2.5">
