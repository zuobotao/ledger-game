...
const effectiveMax = computed(() => {
  if (props.mode === 'buy' && props.availableCash !== undefined && props.unitPrice > 0) {
...
const isCashInsufficient = computed(() => {
  ...
  if (props.availableCash === undefined) return false
  ...
  >
    ...
    <div v-if="showQuickButtons" class="flex flex-wrap justify-center gap-2">
      <button
        ...
        @click="setQuantity(effectiveMax)"
      >
        全部
      </button>
      <button
        class="px-3 py-1.5 rounded-full text-xs bg-secondary text-secondary-foreground hover:bg-accent disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        :disabled="Math.floor(effectiveMax / 2) < 1"
        @click="setQuantity(Math.floor(effectiveMax / 2))"
      >
        一半
      </button>
      <button
    ...
    >
      ...
      <div v-if="mode === 'buy' && availableCash !== undefined" class="flex justify-between items-center text-xs">
